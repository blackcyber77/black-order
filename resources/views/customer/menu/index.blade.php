...
@section('content')
<!-- Hero Section -->
<div class="bg-gradient-to-br from-navy-900 via-navy-800 to-slate-900 text-white min-h-[300px] flex items-center relative overflow-hidden -mt-24 pt-24 pb-12">
    <!-- Abstract Shapes -->
    <div class="absolute top-0 right-0 w-96 h-96 bg-orange-500/20 rounded-full blur-3xl -mr-32 -mt-32 animate-float"></div>
    <div class="absolute bottom-0 left-0 w-64 h-64 bg-blue-500/20 rounded-full blur-3xl -ml-20 -mb-20"></div>

    <div class="container mx-auto px-4 relative z-10">
        <div class="max-w-2xl">
            <h1 class="text-4xl md:text-5xl font-bold mb-4 leading-tight">Lapar? <br><span class="text-orange-500">Pesan Sekarang</span>, Diantar Langsung.</h1>
            <p class="text-slate-300 text-lg mb-8">Nikmati berbagai pilihan kuliner terbaik dari tenant kami, pesan langsung dari meja Anda.</p>
            ...
            <div class="bg-white/10 backdrop-blur-md border border-white/20 p-2 rounded-2xl flex gap-2 max-w-md">
                ...
                @if(!$tableNumber)
                <button onclick="openScanner()" class="bg-orange-500 hover:bg-orange-600 text-white px-6 rounded-xl font-bold transition shadow-lg shadow-orange-500/30">
                    <i class="fas fa-qrcode mr-2"></i> Scan
                </button>
                @else
                <a href="{{ route('location.clear') }}" class="bg-red-50 hover:bg-red-100 text-red-500 px-4 rounded-xl font-bold transition flex items-center justify-center border border-red-200">
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="sticky top-20 z-30 bg-gray-50/95 backdrop-blur-sm py-4 -mx-4 px-4 mb-6 border-b border-gray-200/50">
        <form action="{{ route('home') }}" method="GET" class="flex flex-col md:flex-row gap-4 max-w-4xl mx-auto">
            ...
            <div class="relative flex-1">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari makanan atau minuman..." 
            ...
            <div class="flex gap-2 overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
                <select name="category" onchange="this.form.submit()" class="px-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-500/20 bg-white">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group flex flex-col h-full">
            <!-- Image -->
            <div class="relative aspect-[4/3] overflow-hidden">
                ...
                <div class="absolute top-2 right-2">
                    <span class="bg-white/90 backdrop-blur text-[10px] font-bold px-2 py-1 rounded-full text-navy-900 shadow-sm border border-white/50">
